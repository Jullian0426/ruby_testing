...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      subject(:incomplete_game) { described_class.new(5, '3') }
  ...
  describe '#final_message' do
    ...
    context 'when count is 2-3' do
      subject(:game_few) { described_class.new(5, '5', 3) }

      it 'outputs correct phrase' do
        congrats_phrase = "Congratulations! You picked the random number in 3 guesses!\n"
        expect { game_few.final_message }.to output(congrats_phrase).to_stdout
      end
    end
    ...
    context 'when count is 4 and over' do
      subject(:game_many) { described_class.new(5, '5', 4) }

      it 'outputs correct phrase' do
        hard_phrase = "That was hard. It took you 4 guesses!\n"
        expect { game_many.final_message }.to output(hard_phrase).to_stdout
      end
    end
  end
end
